...
earth_orbit = p.circle(
    x=0, y=0, radius=1, line_color="blue", line_alpha=0.5, fill_color=None, line_width=2
)
mars_orbit = p.circle(
    x=0,
    y=0,
...
sun = p.circle(
    x=0, y=0, radius=0.2, line_color=None, fill_color="yellow", fill_alpha=0.5
)

# Creating columndatasources for the two moving circles
earth_source = ColumnDataSource(
    data=dict(
        x_earth=[earth_orbit.glyph.radius * cos(radians(0))],
        y_earth=[earth_orbit.glyph.radius * sin(radians(0))],
    )
)
mars_source = ColumnDataSource(
    data=dict(
        x_mars=[mars_orbit.glyph.radius * cos(radians(0))],
...
# we will generate x and y coordinates every 0.1 seconds out of angles starting from an angle of 0 for both
# earth and mars
i_earth = 0
i_mars = 0
