...
def extract_value(market_name="btcnCNY"):
    r = requests.get(
        "http://bitcoincharts.com/markets/" + market_name + ".html",
...
def update():
    new_data = dict(
        x=[datetime.now(tz=timezone("Africa/Lagos"))], y=[extract_value(select.value)]
    )
...
options = [("btcnCNY", "BTCN China"), ("okcoinUSD", "Okcoin CNY")]
select = Select(title="Market Name", value="btcnCNY", options=options)
