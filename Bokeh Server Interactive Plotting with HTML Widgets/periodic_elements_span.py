...
liquid_min_boil = min(liquid.data["boiling point"])
liquid_max_boil = max(liquid.data["boiling point"])

# Create three spans
span_gas_average_boil = Span(
    location=gas_average_boil, dimension="width", line_color="yellow", line_width=2
)
span_liquid_average_boil = Span(
    location=liquid_average_boil, dimension="width", line_color="orange", line_width=2
)
span_solid_boil = Span(
    location=solid_average_boil, dimension="width", line_color="red", line_width=2
)
...
def update_liquid_span(attr, old, new):
    span_liquid_average_boil.location = float(select_liquid.value)


"""
Select widgets expect a list of tuples of strings, so List[Tuple(String, String)], that's why you should convert 
average, max, and min to strings
"""
...
select = Select(title="Select Solis state span value", options=options)
select.on_change("value", update_span)


options_liquid = [
    (str(liquid_average_boil), "Liquid Average Boiling Point"),
    (str(liquid_min_boil), "Liquid Minimum Boiling Point"),
    (str(liquid_max_boil), "Liquid Maximum Boiling Point"),
]

# Create the Select widget
select_liquid = Select(title="Select liquid state span value", options=options_liquid)
select_liquid.on_change("value", update_liquid_span)

# Add Select widget to layout and then the layout to curdoc
lay_out = layout([[select, select_liquid]])
curdoc().add_root(f)
